...
{{-- report search form --}}
<form id="frm_report" action="{{ route('damageloss.report') }}" method="GET">
<div class="col-md-12" id="content">
  ...
  @endif
    ...
    <div class="row mt-3 report_filters">
      ...
      {{-- damage checkbox --}}
      <div class="col-sm-1 col-form-label offset-sm-2 pl-4">
        <input type="checkbox" class="form-check-input" name="damage" value="1"  {{ app('request')->input('damage') == config('constants.DAMAGE') ? 'checked' : '' }}>
        <label class="form-check-label" >{{__('Damage')}}</label>
      </div>
      {{-- loss checkbox --}}
      <div class="col-sm-2 col-form-label pl-3">
        <input type="checkbox" class="form-check-input" name="loss" value="2"  {{ app('request')->input('loss') == config('constants.LOSS') ? 'checked' : '' }}>
        <label class="form-check-label" >{{__('Loss')}}</label>
      </div>
    </div>
    @endif
    <div class="row mt-3 report_filters">
      ...
      <div class="col-sm-3">
        <input type="date" class="form-control datetimepicker-input" name="from_date" placeholder="{{__('From Date')}}" value="{{ app('request')->input('from_date') != '' ? app('request')->input('from_date') : '' }}">
      </div>
      <label class="col-sm-2 col-form-label">{{__('To Date')}}</label>
      <div class="col-sm-3">
        <input type="date" class="form-control datetimepicker-input" name="to_date" placeholder="{{__('To Date')}}" value="{{ app('request')->input('to_date') != '' ? app('request')->input('to_date') : '' }}">
      </div>
      ...
      <table id="saleTable" class="table table-bordered text-nowrap">
        <thead class="thead-light">
          <tr>
            ...
            <th>{{__('Damage Loss')}} {{__('Quantity')}}</th>
            <th>{{__('Type')}}</th>
          </tr>
        </thead>
        <tbody>
          @forelse($data as $value)
            ...
            <td>{{$value->damageloss_quantity}}</td>
            @if($value->type == config('constants.DAMAGE'))
            <td> Damage </td>
            @elseif($value->type == config('constants.LOSS'))
            <td> Loss </td>
            @endif
            <tr>
           @empty
