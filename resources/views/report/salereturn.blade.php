...
{{-- report search form --}}
<form id="frm_report" action="{{ route('salereturn.report') }}" method="GET">
...
<div class="card card-info">
    ...
    <div class="table-responsive">
      ...
      <table id="saleTable" class="table table-bordered text-nowrap">
        <thead class="thead-light">
          <tr>
            <th>{{__('Sale')}} {{__('Date')}}</th>
            <th>{{__('Return')}} {{__('Date')}}</th>
        ...
        <tbody>
          @forelse($data as $value)
            <tr>
            <td>{{ date('m/d/Y',strtotime($value->sale_date)) }}</td>
            <td>{{ date('m/d/Y',strtotime($value->return_date)) }}</td>
  ...
  <div class="card-footer" style="{{ (count($data)== 0 ? 'display:none':'')}}">
    {{-- sale return report list pagination size filters --}}
    <nav>
...
<!-- <script src="{{asset('js/report/salereturnreport.js')}}"></script> -->
