...
<form id="frm_shop">
  <div class="col-md-12">
    ...
    <div class="form-group">
      <a href="{{ url('shop/create') }}" class="btn btn_add">{{ __('Add') }}</a>
    </div>

    <div class="card card-info">
      {{-- form title --}}
      <div class="card-header">
        <h3 class="card-title">{{ __('Shop') }} {{ __('List') }}</h3>
      </div>

      <div class="card-body">
        <div class="table-responsive">
         {{-- shop table --}}
          <table id="shopList" class="table table-bordered text-nowrap">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>{{ $shop->phone_number_2 ?? "-"}}</td>
                <td class="text-center"><a href="{{ url('/shop/' . $shop->id . '/edit') }}" class="nav-icon fas fa-edit iconSize"></a></td>
              </tr>
              @empty
              <tr>
      ...
      <div class="card-footer">
        {{-- shop list pagination size filters --}}
        <nav>
        ...
        {{ $shopList->withQueryString()->links() }}
      </div>
    </div>
  </div>
</form>
...
@section("js")
<script type="text/javascript" src="{{asset('js/shop/list.js')}}"></script>
@endsection
